...
def detectCarPlate():
    ...
    global detectedRfid
    while True:
        if module.card_reader.GPIO.input(20)==0 and detectedCheck == False:
            os.system("curl -s -o /dev/null http://localhost:8080/2/action/snapshot")
            time.sleep(0.5)
            frame = cv2.imread("/home/pi/ProjectPython/TwoRC522_2/stream_save/lastsnap.jpg",1)
...
def monthTicketProcess():
    ...
    global detectedRfid
    while True:
        if module.card_reader.GPIO.input(20)!=0:
            detectedCheck = False
        ...
        if detectedCheck == True and module.card_reader.cardSwifted==True:
            if detectedNumber.replace(".","").replace(" ","") == module.card_reader.plateNumber and detectedRfid == module.card_reader.cardRfid:
                module.card_reader.pwm1.ChangeDutyCycle(7.5)
                ...
                db.commit()
                checkDelay = False
                while not (checkDelay and module.card_reader.GPIO.input(20) and module.card_reader.GPIO.input(21)):
                    checkDelay=False
                    if module.card_reader.GPIO.input(20) & module.card_reader.GPIO.input(21):
                        checkDelay=True
                        time.sleep(1)
                time.sleep(1)
                module.card_reader.pwm1.ChangeDutyCycle(13)
                ...
                sql = "insert into parkHistory(plateNumber,UID, fileName, createDate,status) value('%s','%s','%s',now(),1);" % (module.card_reader.plateNumber, module.card_reader.cardRfid, filename)
                cursor.execute(sql)
                db.commit()
                detectedCheck = False;
                module.card_reader.cardSwifted = False
